...
def config(*args, **kw):
    try:
        reporters = getattr(settings, 'DJANGO_POTATO_CAPTCHA_REPORTERS', [])
    except ImportError:
        return
    ...
    for reporter in reporters:
        ...
        process.connect(to, dispatch_uid='captcha.reporter.%s' % reporter)

    # The warning signal is sent by forms, sessions etc when they
    # encounter something.
    warning.connect(add_signal, dispatch_uid='captcha.warning')
    ...
    # on that request.
    finished.connect(process_signals, dispatch_uid='captcha.finished')
